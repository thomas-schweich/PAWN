...
SHA (`777b8807…`, the commit annotated tag `sf_18-v0.2.0` currently points
...
- `option name NetSelection` — required if any tier sets
  `net_selection: <X>`. Older fork builds (`sf_18-v0.1.0`) advertise
...
so a `sf_18-v0.1.0` fork build is sufficient there. Either way, an
