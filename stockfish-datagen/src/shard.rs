...
pub struct GameRow {
    ...
    pub game_seed: u64,
    /// Stockfish's `id name` line, identical for every game generated by
    /// a single worker. Stored as `Arc<str>` so the per-game construction
    /// in the hot loop is a cheap refcount bump instead of a fresh
    /// `String` allocation per row.
    pub stockfish_version: Arc<str>,
...
impl ShardWriter {
    ...
    pub fn append(&mut self, row: &GameRow) {
        ...
        // `&Arc<str>` derefs to `&str` for `StringBuilder::append_value`
        // via the `AsRef<str>` blanket on `&T`.
        self.stockfish_version.append_value(row.stockfish_version.as_ref());
