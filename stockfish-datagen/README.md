...
Stockfish version, `shard_size_games`, `max_ply`, `n_workers`, and
the `SHARD_SCHEMA_VERSION` constant from `src/shard.rs`. Adding or
modifying *other* tiers in the run config does NOT invalidate prior
tiers' manifests.
