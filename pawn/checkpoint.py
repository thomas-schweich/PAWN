...
A checkpoint is a directory:
    ...
    .complete           integrity sentinel — JSON object ``{"files":
                        {filename: sha256-hex}}`` (and historically also
                        ``format_version``; that field is written but not
                        checked here — ``config.json`` is the
...
``safetensors.numpy``. Atomic-write + sentinel helpers live in
``pawn._sentinel`` (shared with ``pawn.torch_loader``).
"""
...
def _write_sentinel(directory: Path) -> None:
    ...
    Preserves the historical ``{"format_version": ..., "files": ...}``
    layout — ``format_version`` is informational (the canonical version
    source is ``config.json``); ``pawn._sentinel.verify_sentinel``
    checks only ``"files"``.
    """
    from pawn._sentinel import sha256_file as _sha256_file
    hashes = {
