...
"""Vendored fused attention with an fp32-softmax-gradient BACKWARD.

VENDORED + PATCHED copy of
``jax/experimental/pallas/ops/gpu/attention.py`` (Apache 2.0, JAX Authors).
The only changes from upstream are marked ``# FP32-BWD patch``: the backward
kernel keeps the softmax-gradient *cancellation* terms (``delta = O·dO``,
``p``, ``ds``) in float32 instead of rounding them to bf16 before the dQ/dK/dV
matmuls. Upstream's bf16 rounding of those terms breaks the score-gradient
cancellation in saturated attention and produced the PAWN pretrain grad-norm
blow-up. Data matmuls (QK, dO·V) stay bf16. Experiment branch
``exp/flash-kernel-fp32-bwd`` — the FUSED-backward alternative to the
custom-VJP (materialised) fix on jax_migration; the question is whether the
fused fp32 terms fix the gradient AND keep flash's speed.
"""
...
def _preprocess_backward_kernel(out_ref, dout_ref, delta_ref, head_dim: int):
  ...
  # compute
  delta = jnp.sum(o.astype(jnp.float32) * do.astype(jnp.float32), axis=1)  # FP32-BWD patch
  # write-back
...
def mha_backward_kernel(
  ...
  def inner_loop_dkdv(start_q, carry):
    ...
    dv = dv + pl.dot(p.T, do.astype(jnp.float32))  # FP32-BWD patch: keep p fp32
    dp = jnp.zeros((block_q_dkv, block_kv_dkv), dtype=jnp.float32) - di[:, None]
    dp = dp + pl.dot(do, v.T)
    ds = p * dp
    if sm_scale != 1.0:
      ds = ds * sm_scale
    dk = dk + pl.dot(ds.T, q.astype(jnp.float32))  # FP32-BWD patch: keep ds fp32
  ...
  def inner_loop_dq(start_k, dq):
    ...
    dq = dq + pl.dot(ds, k.astype(jnp.float32)).astype(dq.dtype)  # FP32-BWD patch: keep ds fp32
