...
def verify_sentinel(directory: Path) -> None:
    ...
    raw_listed = data["files"]
    # Validate the shape before treating ``raw_listed`` as
    # ``dict[str, str]``: a malformed sentinel (e.g. ``files`` is a list,
    # or its values are integers) would otherwise crash ``sha256_file``
    # with an opaque ``TypeError`` instead of raising
    # ``CheckpointIntegrityError``.
    if not isinstance(raw_listed, dict) or not all(
        isinstance(k, str) and isinstance(v, str)
        for k, v in raw_listed.items()
    ):
        raise CheckpointIntegrityError(
            f"{sentinel} is malformed — 'files' must be a JSON object whose "
            f"values are all string SHA-256 digests"
        )
    listed: dict[str, str] = raw_listed
    present = {
        entry.name
        for entry in directory.iterdir()
        if entry.is_file() and entry.name != SENTINEL_FILE
