"""Legacy PyTorch ``PAWNCLM`` — TEST FIXTURE ONLY.

Verbatim relocation of what used to be ``pawn/model.py`` during the
pre-JAX era. Preserved as a **test fixture** for two purposes:

1. ``pawn.legacy.convert_legacy_checkpoint`` parity tests
   (``tests/test_jax_legacy.py``, ``tests/test_jax_torch_loader.py``)
   need a way to (a) build a torch checkpoint with the canonical
   legacy state-dict schema and (b) run a torch forward to compare
   against the converted JAX checkpoint. The schema is exactly this
   module's ``PAWNCLM.state_dict()`` output.
2. ``scripts/convert_published_checkpoints.py`` uses it for the
   end-to-end parity check against the three published HF
   checkpoints.

Architecture is intentionally frozen: the saved state-dict here must
byte-match what the three published checkpoints carry. Do not refactor
this file. Constants + the legacy ``CLMConfig`` are inlined so this
module has no dependency on the deleted ``pawn.config``.
"""
...
# Vocabulary constants — frozen at the published-checkpoint era values.
NUM_ACTIONS = 1968
PAD_TOKEN = 1968
...
@dataclass
class CLMConfig:
    """Architecture hyperparameters for the legacy PyTorch model.

    Verbatim relocation of ``pawn.config.CLMConfig``. The
    ``small/base/large`` factories are kept so existing
    ``CLMConfig.small()`` callers in the converter parity tests
