#!/usr/bin/env python3
"""FiLM behavioral cloning on Lichess games.

Trains FiLM adapters on frozen PAWN to predict human moves from
...
Usage:
    uv run python scripts/train_film.py \
        ...
        --output-dir film_runs/elo_1200_1400
"""
...
def parse_args():
    p = argparse.ArgumentParser(description="FiLM BC on Lichess games")
    ...
    p.add_argument("--no-output-film", action="store_true",
                    help="Skip output logit FiLM; only train hidden-layer adapters")
...
def main():
    ...
    model = FiLMCLM(backbone, use_output_film=not args.no_output_film).to(device)

    # Cache FiLM parameter list (avoid rebuilding every step)
    film_params = model.film_parameters()

    n_film = sum(p.numel() for p in film_params)
    n_total = sum(p.numel() for p in model.parameters())
    print(f"FiLM params: {n_film:,} / {n_total:,} total ({100*n_film/n_total:.3f}%)")
    ...
    optimizer = torch.optim.AdamW(
        film_params, lr=args.lr, weight_decay=args.weight_decay,
    )
    ...
    for epoch in range(args.epochs):
        ...
        for batch in train_loader:
            ...
            # Sparse forward: lm_head only at loss-masked positions
            valid_logits = sparse_forward(model, ids, msk, legal_mask, use_amp, device)
            valid_targets = tgt[msk]
            ...
            if scaler is not None:
                ...
                torch.nn.utils.clip_grad_norm_(film_params, args.max_grad_norm)
        ...
        # Validation (every val_every epochs, or on the last epoch)
        ...
        # Early stopping (only check when we validated)
        if do_val:
