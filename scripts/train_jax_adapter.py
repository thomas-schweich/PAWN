...
Execution order:
  ...
  1. Parse args; resolve the strategy. Validates ``--total-steps > 0``,
     ...
     argument constraints (e.g. ``--lora-rank > 0`` for LoRA / RoSA;
...
def _build_lora(
    ...
    cfg = LoRAConfig(
        rank=args.lora_rank, targets=tuple(args.lora_targets),
...
def _build_film(
    ...
    cfg = FiLMConfig(use_output_film=args.use_output_film)
...
def _build_hybrid(
    ...
    cfg = HybridConfig(
        ...
        film=FiLMConfig(use_output_film=args.use_output_film),
    )
...
def _build_rosa(
    ...
    cfg = RoSAConfig(
        rank=args.lora_rank, targets=tuple(args.rosa_targets),
        alpha=args.lora_alpha,
    )
...
def _strategy_config_dict(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if s == "lora" or s == "hybrid":
        ...
        if s == "hybrid":
            ...
            # awkward.
            d["use_output_film"] = args.use_output_film
        return d
    if s == "film":
        return {"use_output_film": args.use_output_film}
    if s == "unfreeze":
        return {
            "n_unfreeze": args.n_unfreeze,
            "include_lm_head": args.include_lm_head,
...
def _validate_strategy_args(
    ...
    if args.strategy in ("lora", "hybrid", "rosa"):
        if args.lora_rank <= 0:
            raise SystemExit(
                f"--lora-rank={args.lora_rank} must be positive for {args.strategy}"
            )
    ...
    if args.strategy == "sparse" and not 0.0 < args.density <= 1.0:
        raise SystemExit(
            f"--density={args.density} must be in (0, 1]"
        )
    if args.strategy == "specialized_clm":
        ...
        if args.specialized_d_model <= 0 or args.specialized_n_heads <= 0:
            raise SystemExit(
                f"--d-model={args.specialized_d_model} and "
                f"--n-heads={args.specialized_n_heads} must be "
                f"positive"
            )
        if args.specialized_d_model % args.specialized_n_heads != 0:
            raise SystemExit(
                f"--d-model={args.specialized_d_model} must be "
                f"divisible by --n-heads={args.specialized_n_heads}"
            )
        ...
        if args.specialized_d_ff <= 0:
            raise SystemExit(
                f"--d-ff={args.specialized_d_ff} must be positive"
            )
        if args.specialized_n_layers <= 0:
            raise SystemExit(
                f"--n-layers={args.specialized_n_layers} must "
                f"be positive"
            )
...
def _apply_config_json(parser: argparse.ArgumentParser, argv: list[str] | None) -> None:
    """Pre-process argv: if ``--config <path>`` is set, load the JSON,
    validate through ``AdapterConfig`` (pydantic — the single source of
    truth per ``docs/jax-migration.md`` §8.1), and set the loaded
    values as argparse defaults so any explicit command-line flag
    ...
    # Flatten the nested ``specialized_clm`` config (if present) so
    # the argparse defaults can pick up the individual fields.
    nested = data.pop("specialized_clm", None)
    cfg = AdapterConfig(**data, **(
        {"specialized_clm": nested} if nested is not None else {}
    ))
    dumped = cfg.model_dump()
    # Flatten nested specialized_clm dict so its fields map to the
    # ``specialized_d_*`` argparse dests directly.
    nested_dump = dumped.pop("specialized_clm", None)
    if nested_dump is not None:
        for k in ("d_model", "n_layers", "n_heads", "d_ff"):
            if k in nested_dump:
                dumped[f"specialized_{k}"] = nested_dump[k]
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        ...
        help="optional path to a JSON config file. Loaded through "
             "``pawn.run_config.AdapterConfig`` (pydantic — the single "
    ...
    # FiLM / Hybrid shared args. v1 canonical name per §8.3 —
    # ``use_output_film: bool = True`` (the v2 ``--no-film-output``
    # polarity flip was reverted). Both ``--use-output-film`` and
    # ``--no-use-output-film`` are exposed via BooleanOptionalAction
    # so the default-True semantics are preserved and either
    # direction can be expressed explicitly.
    parser.add_argument(
        "--use-output-film",
        dest="use_output_film",
        action=argparse.BooleanOptionalAction,
        default=True,
        help="FiLM / Hybrid: enable γ⊙logits+β modulation after "
             "``lm_head`` (default: True). Pass ``--no-use-output-film`` "
             "to disable; per-layer γ⊙h+β stays on either way.",
    )
    # Unfreeze args.
    parser.add_argument(
    ...
    parser.add_argument(
        "--no-adapt-attn", action="store_true",
        help="Bottleneck: skip the attention-side adapter.",
    )
    parser.add_argument(
        "--no-adapt-ffn", action="store_true",
    ...
    # SpecializedCLM args. v1-canonical names per §8.3 — no
    # ``specialized_`` prefix, since these live inside the
    # per-strategy config where the prefix is redundant. The dest
    # keeps the ``specialized_`` prefix so the namespace doesn't
    # collide with any future top-level ``--d-model`` etc.
    parser.add_argument(
        "--d-model", dest="specialized_d_model", type=int, default=84,
        help="SpecializedCLM: model width.",
    )
    parser.add_argument(
        "--n-layers", dest="specialized_n_layers", type=int, default=2,
        help="SpecializedCLM: number of transformer layers.",
    )
    parser.add_argument(
        "--n-heads", dest="specialized_n_heads", type=int, default=2,
        help="SpecializedCLM: number of attention heads.",
    )
    parser.add_argument(
        "--d-ff", dest="specialized_d_ff", type=int, default=192,
