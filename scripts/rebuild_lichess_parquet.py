#!/usr/bin/env python3
"""Rebuild the Lichess parquet dataset with the current 1,968-action vocabulary.

Streams raw monthly PGN dumps from `thomas-schweich/raw-lichess` directly
(no full local copy of the .zst), reparses each game via the current Rust
chess engine, and incrementally uploads each parquet shard to a new
destination dataset repo as soon as it finishes — deleting the local copy
immediately after a successful upload. Disk usage is bounded to roughly one
in-flight shard per worker.

The existing hosted Lichess datasets were built against a legacy ~60k-entry
move vocabulary. The current engine exports the 1,968-action searchless_chess
vocabulary, which `parse_pgn_lichess` uses implicitly, so simply reparsing the
raw dumps through the Rust bindings produces the new tokens.
...
ignore sentinels and rebuild everything.

Branching
---------
By default the script writes every shard, sentinel, and cleanup commit to a
`run/rebuild` branch (created automatically). `main` stays untouched until
...
branch name to run multiple rebuilds in parallel.
...
Games longer than `--max-ply` are truncated; their outcome slot is set to
...
-----
    ...
    uv run python scripts/rebuild_lichess_parquet.py \\
        --months 2025-01 2025-02 2025-03 \\
        --holdout-month 2026-01 \\
        --dest-repo thomas-schweich/pawn-lichess-v3 \\
        --jobs 4
"""
...
DEFAULT_REVISION = "run/rebuild"
DEFAULT_SCRATCH_DIR = Path("/dev/shm/pawn-lichess-rebuild")
...
# ---------------------------------------------------------------------------
# Parsed batch -> Polars DataFrame (schema mirrors extract_lichess_parquet v2)
# ---------------------------------------------------------------------------
...
def delete_stale_shards(
    ...
    api.create_commit(
        ...
        commit_message=f"Clean up {len(paths)} orphan file(s) before rebuild",
    )
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Rebuild Lichess parquet dataset with the current 1,968-action vocabulary"
    )
    parser.add_argument(
        "--months", nargs="+", required=True,
        help="Training months to rebuild, e.g. 2025-01 2025-02 2025-03",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "branch. Squash-merge this branch to main once the rebuild is verified."
        ),
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help="Ignore existing sentinels and rebuild every requested unit",
    )
    ...
    log("=== Lichess parquet rebuild (new 1,968-action vocabulary) ===")
