...
Phase-3 driver. Loads the supernet, slices to a named variant, wraps with a LoRA adapter, and runs the two-tier frozen-backbone / trainable-adapter optimisation under a K-step `lax.scan`. The driver currently dispatches LoRA only; the other 7 strategy *modules* (`film` / `unfreeze` / `bottleneck` / `hybrid` / `sparse` / `rosa` / `specialized_clm`) are ported and live in `pawn/adapters/`, but the `--strategy` CLI glue is a pending follow-up chunk — see `docs/jax-migration.md` §12.
...
The verification proxy is a Rust-engine random-game corpus. The broader adapter strategies are ported in `pawn/adapters/`; their dispatch glue (the `--strategy` flag plus per-strategy hyperparameter args, and RoSA's three-phase training schedule) lands in the pending follow-up chunk described at `docs/jax-migration.md` §12. The Lichess Elo-stratified eval landed under `pawn.lichess_eval`.
