...
```bash
# Tiny verification run (default --strategy lora)
uv run python scripts/train_jax_adapter.py \
    --supernet tiny --variant base --rank 4 --total-steps 500

# RoSA three-phase
uv run python scripts/train_jax_adapter.py --strategy rosa \
    --supernet tiny --variant base --rank 4 --total-steps 1000 \
    --rosa-warmup-frac 0.4 --rosa-top-k-frac 0.01

# specialized_clm (from-scratch; no backbone)
uv run python scripts/train_jax_adapter.py --strategy specialized_clm \
    --specialized-d-model 64 --specialized-n-layers 2 --specialized-n-heads 2 \
    --specialized-d-ff 128 --total-steps 1000
```
...
The verification proxy is a Rust-engine random-game corpus. The Lichess Elo-stratified eval landed under `pawn.lichess_eval`.
...
- `jax_run_<YYYYMMDD>_<HHMMSS>_<microseconds>_<pid>/` — `scripts/train_jax.py` (pretraining). Writes `metrics.jsonl` (one JSON record per scan chunk) and `config.json` (every `ModelConfig` field for the supernet + every variant, the resolved CLI flags, and the run seeds).
- `jax_adapter_run_<...>/` — `scripts/train_jax_adapter.py`. Writes `metrics.jsonl` (one row per chunk; adds val-loss columns on val chunks) and `config.json` (supernet + selected variant cfg + strategy-specific config keys).
