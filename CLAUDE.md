...
│   ├── eval_suite/              # OPTIONAL EXTRA (`data-tools`): legacy position-parquet pipeline (bounds, viz, corpus)
...
# Run the base + JAX test suite. The full suite (eval_suite legacy +
# lab + dashboard tests) needs the matching extras as well.
uv run --extra rocm pytest tests/                # core + JAX + sweep
uv run --extra rocm --extra data-tools --extra lab --extra dashboard \
...
**Data source.** Two paths. The realistic adapter task — human-move prediction at a target Elo — is `--pgn <hf-repo|path>`: the canonical pre-tokenized Lichess parquet (`thomas-schweich/pawn-lichess-full`), optionally narrowed to an Elo band via `--elo-min` / `--elo-max`. `pawn.lichess_data` filters + packs it into a `Corpus` and caches the result under `$HF_HOME/pawn-lichess-cache/<key>/`. The finite slice is tiled across epochs (per-epoch permutation) to fill `total_steps × batch_size` game-slots. Omitting `--pgn` falls back to Rust-engine random games — a verification proxy, not a real adapter benchmark. The `--pgn` path needs the `data-tools` extra (polars).
...
- `--pgn`, `--elo-min`, `--elo-max`, `--min-ply`, `--max-games`, `--pgn-split`, `--cache-dir` — Lichess data source (omit `--pgn` for the random-game proxy)
...
Off-the-hot-path tooling kept behind the `--extra data-tools`: theoretical accuracy bounds (`bounds.py`), matplotlib/seaborn plotting (`viz.py`), polars parquet iterator (`corpus.py`). The JAX surface above does not depend on any of it.
...
The runtime images bake `--extra data-tools` into the `uv sync` line so the published image can run `extract_lichess_parquet`, `compute_theoretical_ceiling`, `generate_model_cards` and the legacy `pawn.eval_suite` pipeline without a manual sync. Dashboard / lab / wandb extras are not in the runtime image; the `:dev` tags include them.
