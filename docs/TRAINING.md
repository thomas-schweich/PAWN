...
- **Val games**: validation source follows the carve/held-out gate (`pawn.lichess_data.load_lichess_train_val`). An explicit `--pgn-val-split <name>` is used directly; otherwise, when the dataset publishes a `validation` split with files (probed by `split_has_files`), that held-out split is used; otherwise — `--pgn-val-split ""`, or a single-file / split-less source — a deterministic slice is carved out of the training games. The carve size matches the v1 cap formula (`scripts/train.py`): `n_val = min(val_games, n_total // 5)` — a 20% floor-division carve, capped by `--val-games` when set, clamped to keep at least one training game. The split is a seeded permutation (the run `data_seed`), so the carved validation slice and the returned training corpus are guaranteed **disjoint** and reproducible across runs.
...
Adapter training filters, tokenizes, and packs the dataset to disk before the first step, then loads the result. The cache key is a SHA-256 of the filter params (source, split, Elo bounds, min_ply, seq_len, conditioning, plus the cache-format and loss-mask versions) — different filter combinations produce different cache directories, with the cache root resolved as `$PAWN_DATA_CACHE` (used verbatim) → `$HF_HOME/pawn-lichess-cache` → `~/.cache/huggingface/pawn-lichess-cache` (override per-call with `cache_dir`). Because `seq_len` and `conditioning` fold into the key, changing the packing layout produces a fresh entry rather than serving a mismatched corpus.

The v2 cache stores one `corpus.safetensors` per entry — a dense `(N, T)` int32 token matrix plus the `targets` / `attn_mask` / `loss_mask` / `outcome_offset` / `game_lengths` arrays — not the v1 CSR layout (flat int16 buffer + offsets). A `.complete` SHA-256 sentinel guards the directory; every load verifies it and a corrupted entry is rebuilt on next launch.
